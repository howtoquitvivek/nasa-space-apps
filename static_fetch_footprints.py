...
# Assume 256x256 grayscale tiles → 65536 bytes ≈ 64 KB
BYTES_PER_TILE = 256 * 256


def get_capabilities_url_from_tile_url(tile_url):
    """Convert tile URL to capabilities XML URL"""
...
def extract_zoom_levels(xml_root):
    """Extract zoom levels from WMTS capabilities"""
    if xml_root is None:
        return []
    ...
    return sorted(zoom_levels, key=lambda x: x["zoomLevel"])


def lonlat_to_tile(x, y, zoom, bbox_full):
    """Convert lon/lat (or Mars coords) to tile indices for a given zoom"""
...
def calculate_sizes(zoom_levels, bbox, bbox_full, ranges=[(0, 3), (0, 6), (0, 7)]):
    """Calculate bbox-specific download sizes"""
    sizes = {}
    ...
    # Predefined ranges
    for min_zoom, max_zoom in ranges:
        total_bytes = 0
        for zoom in zoom_levels:
            zl = zoom["zoomLevel"]
            if min_zoom <= zl <= max_zoom:
                # Convert bbox corners → tile indices
                ...
                total_bytes += tile_count * BYTES_PER_TILE

        sizes[f"{min_zoom}-{max_zoom}"] = {
            "bytes": total_bytes,
            "readable": human_readable(total_bytes)
        }

    # Full zoom range (instead of "total")
    if zoom_levels:
        zmin, zmax = zoom_levels[0]["zoomLevel"], zoom_levels[-1]["zoomLevel"]
        total_bytes = 0
        for zoom in zoom_levels:
            zl = zoom["zoomLevel"]
            tx_min, ty_max = lonlat_to_tile(minx, miny, zl, bbox_full)
...
def fetch_and_process_footprints(limit=None):
    """Fetch CTX footprints and calculate bbox-specific zoom level sizes"""
    ...
    for i, item in enumerate(footprint_docs):
        ...
        try:
            # Get layer service for tile endpoint
            ...
            tile_url = f"{tile_endpoint}/1.0.0/default/default028mm/{{z}}/{{y}}/{{x}}.png"
            ...
            footprint = {
                ...
                "bbox": bbox,
                "tileUrl": tile_url,
                "capabilitiesUrl": get_capabilities_url_from_tile_url(tile_url)
            }

            # Fetch WMTS XML
            cap_url = footprint["capabilitiesUrl"]
            xml_root = ET.fromstring(requests.get(cap_url).content)
            zoom_levels = extract_zoom_levels(xml_root)

            if zoom_levels:
                # Mars global bbox (Trek usually uses -180,180 but adjust if 0–360)
                bbox_full = [-180, -90, 180, 90]
                sizes = calculate_sizes(zoom_levels, bbox, bbox_full)
                footprint["downloadInfo"] = {
                    "zoomLevels": zoom_levels,
                    "sizes": sizes
                }
                for r, s in sizes.items():
                    print(f"    - Zoom {r}: {s['readable']}")
            else:
